 ...
 *------------------------------------------------------------------------------
 *
 *  Hide Completed Tournament Columns
 *
 */

require(['dom', 'directive', 'node'], function(dom, directive, node) {
    ...
    let columns = null,
        modifier = "hidden",
        ref = "tournament-round-complete";


    const toggle = () => {
        if (columns === null) {
            columns = dom.refs(ref);
        }

        node.classname(columns, { [modifier]: 'toggle' });
    };


    directive.on('tournament.columns', toggle);
 ...
 *------------------------------------------------------------------------------
 *
 *  Tournament Bracket Team Hover Effect
 *
 */
...
require(['dom', 'directive', 'node'], function(dom, directive, node) {
    ...
    const find = function(id) {
        if (id in nodes === false) {
            let parsed = {
                    all:  [],
                    last: []
                },
                refs = dom.refs(`tournament-team-${id}`);

            for (let i = 0, n = refs.length; i < n; i++) {
                let team = refs[i],
                    match = team.parentElement;

                if ((i + 1) === n) {
                    parsed.last.push(match);
                }
                else {
                    parsed.all.push(match);
                }

                parsed.all.push(team);
            }
    ...
    const hover = function() {
        let nodes = find(this.dataset.id);

        if (!nodes) {
            return;
        }

        node.classname(nodes.all,  { 'active': 'toggle' });
        node.classname(nodes.last, { 'active--last': 'toggle' });
    };


    directive.on('tournament.team', hover);
